...
def evaluate(args, model, tokenizer, prefix="",num_steps=1e6,calib_data=True):
    dataset, examples, features = load_and_cache_examples(args, tokenizer, evaluate= not calib_data, output_examples=True)
    ...
    for ii,batch in enumerate(tqdm(eval_dataloader, desc="Evaluating")):
        model.eval()
        batch = tuple(t.to(args.device) for t in batch)
        if ii>num_steps:
            break
    ...
    dataset=examples=features=predictions=None
    return results
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.calib_file)):
            try:
            ...
            if evaluate:
                ...
                examples = processor.get_dev_examples(args.data_dir, filename=args.calib_file)                
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        def hook(module, input, output):
            ...
            # Meanwhile store data in the RAM.
            cached_input_output[module].append(([inp.detach().cpu() for inp in input], output.detach().cpu()))
        ...
        for checkpoint in checkpoints:
            ...
            model = search_replace_layer(model,args.names_sp_layers,num_bits_activation=config.num_bits,num_bits_weight=config.num_bits_weight)
            if args.optimize_weights:
                handlers = []
                count = 0
                for m in model.modules():
                    if isinstance(m, QLinear) or isinstance(m, QMatmul) or isinstance(m, QEmbedding):
                        m.quantize = False
                ...
                # Evaluate
                result = evaluate(args, model, tokenizer, prefix=global_step,num_steps=20)
                for handler in handlers:
                    handler.remove()  
                for m in model.modules():
                    if isinstance(m, QMatmul) or isinstance(m, QLinear) or isinstance(m,QEmbedding):
                        m.quantize = True
                input_size={}
                for i, layer in enumerate(cached_input_output):  
                        if isinstance(layer,QEmbedding):
                            input_size[layer.name] =  {'replaced layer': layer.name,'bandwidth': cached_input_output[layer][0][1][0].numel()}
                        if isinstance(layer,QMatmul):  
                            _,h,d1,d2 = cached_input_output[layer][0][0][0].shape             
                            input_size[layer.name]={'replaced layer': layer.name,'bandwidth': h*d1*d2}
                        else:
                            input_size[layer.name]={'replaced layer': layer.name,'bandwidth': cached_input_output[layer][0][0][0].numel()}
                all_8_dict={'replaced layer': '-','bandwidth': '-'}                            
                columns = [key for key in all_8_dict]
                with open(args.output_dir+'/per_layer_bandwidth.csv', "w") as f:
                ...
                for i, layer in enumerate(cached_input_output):
                    ...
                    print("\nOptimize {}:{} for {} bit of shape {}".format(i, layer.__class__.__name__, layer.num_bits, weight_shape))
                    ...
                    if i > 0 and i % print_freq == 0:
                        print('--------------------',i,'\n')
                        val_results = evaluate(args, model, tokenizer, prefix=global_step,calib_data=False)
